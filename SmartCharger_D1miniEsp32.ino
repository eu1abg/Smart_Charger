...
//TimerMs tmr7(30000, 0, 1);   // таймер проверки вайфай

//=====================================================================================================================
GyverPID regulator(1, 1, 1);  // регулятор охлаждения коэф. П, коэф. И, коэф. Д, период дискретизации dt (мс)
GyverPID regulatorTok(5, 2, 1,1000);  // регулятор заряда БОЛЬШЕ НЕ ТРОГАЕМ
...
void action() { 
 ...
 if (ui.update()) { 
  ...
  //============================= Прием данных ============================================================== 
   ...
   if (ui.click("btn2")) { btn1 = 0; stroka = "Сделайте выбор";flag = 0;ledcWrite(3,0); ledcWrite(2,0); }  //  стоп 
   if (ui.click("sel1")) {valSelect1 = ui.getInt("sel1"); }  //flag =0; hour = 0; minute = 0; second = 0; emk = 0;
    switch (valSelect1) {
...
if (portal==1) {
 ...
 switch (portalStatus()) {
  ...
  oled.setCursor(0, 3);oled.println(portalCfg.pass); EEPROM.put(50,portalCfg.pass); EEPROM.commit(); oled.setScale(1); delayFunction(3000);
   goto label0;  break;
...
oled.clear(); oled.setScale(1);// oled.setCursor(25, 0);oled.invertText(1);oled.print(WiFi.localIP());oled.invertText(0);
  ...
  regulator.setLimits(70, 235);    // пределы (ставим для 8 битного ШИМ). ПО УМОЛЧАНИЮ СТОЯТ 0 И 255
  regulator.setpoint = 40;        // сообщаем регулятору температуру радиатора
  //ledcWrite(0,125); delay(2000); ledcWrite(0,0);
...
if (portal==2) { oled.setScale(1); oled.setCursor(25, 0);oled.invertText(1);oled.print("AP 192.168.1.1");oled.invertText(0);}
  else {oled.setScale(1); oled.setCursor(25, 0);oled.invertText(1);oled.print(WiFi.localIP());oled.invertText(0);}
  ...
  if (tmr5.tick()) { Itok(); }
  if (tmr4.tick()) {Temp(); } regulator.input = trad;  ledcWrite(0,regulator.getResultTimer());
...
//..................................... НИЧЕГО не ВЫБРАНО ..................................................
    ...
    iust = 0; vust =0; led = 0; emk = 0; ledcWrite(3,0); ledcWrite(2,0); }
    if ((btn1 == 0)&&(valSelect2 !=0)&&(flag == 1)) { led = 0; Led1(); ledcWrite(3,0); ledcWrite(2,0);  }   // stroka = "ПАУЗА !!!";


//..................................... Ручная зарядка ..................................................
if ((valSelect2 == 1)&&(btn1 == 1)) { TIMER(); ledcWrite(2,0); stroka ="ИДЕТ ЗАРЯД"; vustgrap = vust;   //  &&(flag == 1) flag = 0; Itok();
 regulatorTok.setpoint = valSpin1; regulatorTok.input = tok; ledcWrite(3,regulatorTok.getResultTimer());
 Emkost(); 
 Led();
 if(vbat >= vust ) { stroka ="Заряд закончен!"; btn1 = 0;flag = 1; ledcWrite(3,0);}
}  

///..................................... Ручная разрядка ..................................................
if ((valSelect2 == 2)&&(btn1 == 1)) { TIMER();ledcWrite(3,0);stroka ="Разряд ИДЁТ!"; vustgrap = vmin;
 regulatorTok.setpoint = valSpin1; regulatorTok.input = -tok; ledcWrite(2,regulatorTok.getResultTimer()); delayFunction(1);  

if (vbat <= vmin) {btn1 = 0;stroka ="Разряд закончен!"; flag = 1; ledcWrite(2,0);}
Emkost();Led();  
}//  Serial.println(valSpin); 
//..................................... АВТО заряд ..................................................
if ((valSelect2 == 3)&&(btn1 == 1)) { TIMER();  ledcWrite(2,0); stroka ="Авто.Заряд ИДЁТ! током 1/10 емкости."; vustgrap = vust;
  if (vbat < vust) { regulatorTok.setpoint = valSpin / 10; valSpin1 = valSpin / 10;  regulatorTok.input = tok; ledcWrite(3,regulatorTok.getResultTimer());delayFunction(1);}
   else {  stroka ="Авто.Заряд ИДЁТ! Стабильным напряжением!"; regulatorTok.setpoint = vust; regulatorTok.input = vbat;ledcWrite(3,regulatorTok.getResultTimer());delayFunction(1); iust = tok;
   if (tok < (valSpin / 50)) { btn1 = 0; stroka ="Авто.Заряд закончен!"; flag = 1; ledcWrite(3,0);} }
 Emkost(); Led();
  }
//..................................... АВТО разряд .................................................. 
if ((valSelect2 == 4)&&(btn1 == 1)) { TIMER(); ledcWrite(3,0); stroka ="Авто.Разряд ИДЁТ!"; vustgrap = vmin;
 regulatorTok.setpoint = valSpin / 10; valSpin1 = valSpin / 10;   regulatorTok.input = -tok; ledcWrite(2,regulatorTok.getResultTimer());delayFunction(1);
  Emkost(); Led();
if (vbat <= vmin ) {btn1 = 0;stroka ="Разряд закончен!"; flag = 1; ledcWrite(2,0);}
}
//..................................... АВТО  ВОСТАНОВЛЕНИЕ .................................................. 
if ((valSelect2 == 5)&&(btn1 == 1)) { vustgrap = vust;
  TIMER(); stroka ="Десульфатация ИДЁТ! Первый этап.";ledcWrite(2,0);
   if ((vbat < vust)&&(flag == 0)) {  regulatorTok.setpoint = valSpin / 10; valSpin1 = valSpin / 10;  regulatorTok.input = tok; ledcWrite(3,regulatorTok.getResultTimer());delayFunction(1);}
         else { flag = 1;
          if (hour < 24) { 
      if (vbat < 16.5) {stroka ="Второй этап Десульфатации! Vbat < 16.5 В.";regulatorTok.setpoint = valSpin / 20; valSpin1 =regulatorTok.setpoint; regulatorTok.input = tok; ledcWrite(3,regulatorTok.getResultTimer());delayFunction(1);}
      ...
      if (vbat > 16.6) { stroka ="Ограничение по напряжению! Vbat > 16.6 В."; regulatorTok.setpoint = 0.1; regulatorTok.input = tok; ledcWrite(3,regulatorTok.getResultTimer());delayFunction(1);}    //  valSpin / (vbat -16.4)*2500
       }
       else {stroka = "Цикл востановления завершен!"; analogWrite(5,0); ledcWrite(3,0); btn1 = 0; flag = 1;}
       }
...
if (second > 5){btn1 = 0; stroka =" Внутр. сопротивление рассчитано! "; } }
...
//===========================================================================================
 ...
 //===========================================================================================
 void Vbat(){ for(int q =1; q<11 ; q++) { vs = vs+(ADS.readADC(1)*f*7.75);  } vbat = vs/10; vs =0; }   // функция измерение напр АКБ
 //=====================================================================================================
 void Vin(){for(int q =1; q<4 ; q++) { vs1 = vs1+(ADS.readADC(3)*f*7.75);} vin = vs1/3; vs1 =0; }     // функция измерение напр Вход
//======================================================================================================
 void Itok(){ for(int q =1; q<51 ; q++) { vs2 = vs2+(8779-ADS.readADC(2));}  tok = (vs2/50)/212; vs2 =0;  } // функция измерение ТОК
//======================================================================================================
...
//======================================================================================================
void Led() {  if (tmr6.tick()) { ledcWrite(1,255); delayFunction(2); ledcWrite(1,0);} } // индикация ждущий
//======================================================================================================
void Led1() { for(int i=0;i<250;i=i+50){ledcWrite(1,i);delayFunction(1);} ledcWrite(1,0);}                                   // индикация заряд  оконченdelayFunction(5);
//======================================================================================================
//void Led2() {  analogWrite(14,n); }        // индикация ждущий
//======================================================================================================
void Sopr() { ledcWrite(3,0); Vbat(); v1 = vbat;  ledcWrite(2,255); 
 Itok();delayFunction(1000); Vbat(); sopr = (v1 - vbat)/-tok; v1=0; ledcWrite(2,0); }                                     // измерение внутр сопр 
//======================================================================================================
void build() {
